...
```{r}
quantiled_data_splited=split(quantiled_data, quantiled_data$quantile)
quantiled_data_splited
```

## Prirodzený nárast celkovo

```{r}
x=quantiled_data[-1,]$`Vzdialenosť od Trnavy(km)`
y=quantiled_data[-1,]$`nárast`

fit <- lm( `Vzdialenosť od Trnavy(km)` ~ `nárast` , quantiled_data[-1,])
...
```{r}
lm(`Vzdialenosť od Trnavy(km)`  ~ `nárast`, data=quantiled_data[-1,])
lm(`nárast` ~ `Vzdialenosť od Trnavy(km)` , data=quantiled_data[-1,])
```
...
abline(lm(temp[[1]][-1,]$`Vzdialenosť od Trnavy(km)` ~ temp[[1]][-1,]$nárast), col="Blue")
